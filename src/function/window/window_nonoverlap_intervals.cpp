...
public:
	explicit WindowNonOverlapIntervalsLocalState(ExecutionContext &context, const WindowNonOverlapIntervalsGlobalState &grstate)
	    : WindowExecutorBoundsLocalState(context, grstate) { // grstate(grstate)
	}

	// Store last interval in global state
	void Sink(ExecutionContext &context, DataChunk &sink_chunk, DataChunk &coll_chunk, idx_t input_idx, OperatorSinkInput &sink) override {
...
WindowNonOverlapIntervalsExecutor::WindowNonOverlapIntervalsExecutor(
    BoundWindowExpression &wexpr, WindowSharedExpressions &shared)
...
unique_ptr<GlobalSinkState> WindowNonOverlapIntervalsExecutor::GetGlobalState(
    ClientContext &client, const idx_t payload_count, const ValidityMask &partition_mask,
    const ValidityMask &order_mask) const {
    return make_uniq<WindowNonOverlapIntervalsGlobalState>(
        client, *this, payload_count, partition_mask, order_mask);
}


void WindowNonOverlapIntervalsExecutor::EvaluateInternal(ExecutionContext &context, DataChunk &eval_chunk, Vector &result,
...
//    auto rdata= FlatVector::GetData<bool>(result);
    ...
    for (idx_t partition_idx = 0; partition_idx < count; ++partition_idx, ++row_idx) {
		...
		struct_entries[3]->SetValue(partition_idx,
			                            Value::INTEGER(NumericCast<int32_t>(partition_idx < 4 ? 3 : pend)));
...
void WindowNonOverlapIntervalsExecutor::Finalize(ExecutionContext &context,
